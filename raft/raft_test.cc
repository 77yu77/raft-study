...
TEST_CASE(part3, unreliable_figure_8,
          "Raft paper Figure 8 under unreliable network") {
  int num_nodes = 5;
  list_raft_group *group = new list_raft_group(num_nodes);
  group->set_reliable(false);
...
void push_kv_commands(kv_raft_group *group, int num, int begin) {
  ...
  for (int i = begin; i < begin + num; i++) {
    ASSERT(leader == group->check_exact_one_leader(),
           "Leader should not change");
    kv_command cmd(kv_command::CMD_PUT, "k" + std::to_string(i),
                   "v" + std::to_string(i * 10));
    int term, index;
    puts.push_back(cmd.res);
    ASSERT(group->nodes[leader]->new_command(cmd, term, index),
           "Leader should not change");
  }
  for (int i = begin; i < begin + num; i++) {
    ...
    if (!puts[i]->done) {
      ASSERT(
          puts[i]->cv.wait_until(lock, std::chrono::system_clock::now() +
    ...
    ASSERT(puts[i]->key == ans_key, "Put command key doesn't match, "
                                        << ans_key << " != " << puts[i]->key);
    ASSERT(puts[i]->value == ans_val, "Put command value doesn't match, "
                                          << ans_val
                                          << " != " << puts[i]->value);
  }
}
...
void put_kv_pair(kv_raft_group *group, int k, bool succ) {
         ...
         "Leader should not change");
  {
    ...
    if (!cmd.res->done) {
      ASSERT(
          cmd.res->cv.wait_until(lock, std::chrono::system_clock::now() +
                                           std::chrono::milliseconds(2500)) ==
              std::cv_status::no_timeout,
          "Put command timeout");
    }
    ...
    if (succ) {
      ...
      ASSERT(cmd.res->value == ans_val,
             "Wrong value " << cmd.res->value << " vs " << ans_val);
    }
  }
}
...
void check_kv_pair(kv_raft_group *group, int k, bool succ) {
         ...
         "Leader should not change");
  {
    ...
    ASSERT(cmd.res->succ == succ,
           "Get command fails " << cmd.res->succ << " vs " << succ);
    if (succ) {
      ASSERT(cmd.res->key == ans_key, "Wrong key");
      ASSERT(cmd.res->value == ans_val, "Wrong value");
    }
...
TEST_CASE(part5, basic_kv, "Basic key-value store") {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (!reads[i]->done) {
      ASSERT(
          reads[i]->cv.wait_until(lock, std::chrono::system_clock::now() +
                                            std::chrono::milliseconds(2500)) ==
              std::cv_status::no_timeout,
          "Get command timeout");
    }
    ...
    std::string ans_key = "k" + std::to_string(i);
    std::string ans_val = "v" + std::to_string(i * 10);
    ASSERT(reads[i]->key == ans_key, "Get command key doesn't match, "
                                         << ans_key << " != " << reads[i]->key);
    ASSERT(reads[i]->value == ans_val, "Get command value doesn't match, "
                                           << ans_val
                                           << " != " << reads[i]->value);
  }
  ...
  ASSERT(group->nodes[leader]->new_command(del_cmd, term, index),
         "Leader should not change");
  ASSERT(group->nodes[leader]->new_command(read_cmd, term, index),
         "Leader should not change");
  {
    std::unique_lock<std::mutex> lock(del->mtx);
    if (!del->done) {
      ASSERT(del->cv.wait_until(lock, std::chrono::system_clock::now() +
                                          std::chrono::milliseconds(2500)) ==
                 std::cv_status::no_timeout,
             "Del command timeout");
    }
    ASSERT(del->key == read_key,
           "Del command key doesn't match, " << read_key << " != " << del->key);
    ASSERT(del->succ, "Del command does not succ?");
  }
  {
    std::unique_lock<std::mutex> lock(rd->mtx);
    if (!rd->done) {
      ASSERT(rd->cv.wait_until(lock, std::chrono::system_clock::now() +
                                         std::chrono::milliseconds(2500)) ==
                 std::cv_status::no_timeout,
             "Get command timeout");
    }
    ASSERT(rd->key == read_key,
           "Get command key doesn't match, " << read_key << " != " << rd->key);
