...
template <typename command> raft_storage<command>::~raft_storage() {
  // Your code here
}
template <typename command> bool raft_storage<command>::defineId(int id) {
  mtx.lock();
...
template <typename command> bool raft_storage<command>::havePresisted() {
  return havePresist;
}
template <typename command> Metadata raft_storage<command>::getMetadata() {
  return metadata;
}
template <typename command>
