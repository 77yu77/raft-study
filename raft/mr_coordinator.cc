...
mr_protocol::status Coordinator::askTask(int,
  ...
  if (!isFinishedMap()) {
    this->mtx.lock();
    printf("askTask\n");
    for (int i = 0; i < (this->mapTasks.size()); i++) {
      printf("find %d\n", i);
    ...
    this->mtx.lock();
    printf("askTask\n");
    for (int i = 0; i < this->reduceTasks.size(); i++) {
...
mr_protocol::status Coordinator::submitTask(int taskType, int index,
                                            bool &success) {
  // Lab2 : Your code goes here.
  ...
  if ((mr_tasktype)taskType == mr_tasktype::MAP) {
    ...
    this->mtx.unlock();
  } else if ((mr_tasktype)taskType == mr_tasktype::REDUCE) {
    this->mtx.lock();
