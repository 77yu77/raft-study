...
getattr(chfs_client::inum inum, struct stat &st)
{
    ...
    if(chfs->isfile(inum)){
       ...
       chfs_client::fileinfo syminfo;
       ret=chfs->getfile(inum,syminfo);
       if(ret!=chfs_client::OK)
         return ret;
        st.st_mode = S_IFLNK | 0777;
        st.st_nlink = 1;
        st.st_atime = syminfo.atime;
        st.st_mtime = syminfo.mtime;
        st.st_ctime = syminfo.ctime;
        st.st_size = syminfo.size;
        printf("   getattr -> %llu\n", syminfo.size);     
...
fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent,const char *name) {
    ...
    r = chfs->symlink(parent,name,link, ino_out);
    if(r == chfs_client::OK) {
        e.ino = ino_out;
