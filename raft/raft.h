...
private:
    ...
    std::vector<int> nextIndex; //对于每个服务器，下一个日志条目的索引 发送给该服务器的索引
    ...
    background_apply(nullptr)
{
    ...
    if(typeid(this->state)==typeid(kv_state_machine *)){
      ...
      std::string path=storage->dir+"kv_store"+std::to_string(my_id)+".txt";
...
int raft<state_machine, command>::request_vote(request_vote_args args, request_vote_reply& reply) {
    ...
    if(current_term<args.term){
     ...
     //RAFT_LOG("arg.lastLogterm %d arg.lastlogindex %d logs.lastlogterm %d logs.lastlogindex %d",args.lastLogTerm,args.lastLogIndex,logs.terms.back(),logs.lastIndex());
    if((votedFor==-1)||(args.lastLogTerm>logs.terms.back())||((args.lastLogTerm==logs.terms.back())&&(args.lastLogIndex>=logs.lastIndex()))){
...
int raft<state_machine, command>::append_entries(append_entries_args<command> arg, append_entries_reply& reply) {
    ...
    if((logs.size()<=arg.prevLogIndex)||(logs.terms[arg.prevLogIndex]!=arg.prevLogTerm)){
      reply.success=false; 
    }
    else{
      // RAFT_LOG("leadindex %d",arg.leaderCommit); 
...
void raft<state_machine, command>::run_background_apply() {
    ...
    while (true) {
        ...
        if(commit_index>last_applied){   
          ...
          for(int i=last_applied+1;(i<=commit_index)&&(i<=logs.lastIndex());i++){
             state->apply_log(logs.entries[i]); 
          }          
          last_applied=commit_index>logs.lastIndex()?logs.lastIndex():commit_index;    
        } 
        mtx.unlock();
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }    
