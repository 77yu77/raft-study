...
void Worker::doMap(int index, const vector<string> &filenames) {
  ...
  for (int j = 0; j < datas.size(); j++) {
    fprintf(fp[hashMap(datas[j].key)], "%s %s ", datas[j].key.data(),
            datas[j].val.data());
  }
  for (int i = 0; i < 4; i++) {
    fclose(fp[i]);
...
void Worker::doSubmit(mr_tasktype taskType, int index) {
  ...
  mr_protocol::status ret =
      this->cl->call(mr_protocol::submittask, taskType, index, b);
...
int main(int argc, char **argv) {
  if (argc != 3) {
    fprintf(stderr,
            "Usage: %s <coordinator_listen_port> <intermediate_file_dir> \n",
            argv[0]);
    exit(1);
  }
